...
// Get formatted date for storage (DDMMYYYY)
function getFormattedDateForStorage() {
    const dateInput = document.getElementById('dateInput');
...
function saveHomeworkToStorage() {
    ...
    const date = getFormattedDateForStorage();
...
// Load homework from localStorage for selected date
function loadHomeworkFromStorage() {
    const date = getFormattedDateForStorage();
    const allHomework = JSON.parse(localStorage.getItem('homeworkData') || '{}');
    
    if (allHomework[date]) {
        const homeworkData = allHomework[date].periods;
        
        // Clear all current homework
        document.querySelectorAll('.homework-list').forEach(list => {
            list.innerHTML = '';
        });
        
        // Populate homework for each period
        homeworkData.forEach(periodData => {
            const container = document.querySelector(`[data-period="${periodData.period}"]`);
            if (container) {
                const homeworkList = container.querySelector('.homework-list');
                
                periodData.homework.forEach(homeworkText => {
                    const homeworkItem = createHomeworkItem(homeworkText);
                    homeworkList.appendChild(homeworkItem);
                });
            }
        });
...
async function submitHomework() {
    ...
    try {
        ...
        setTimeout(() => {
            const success = saveHomeworkToStorage();
            
            if (success) {
                // Show success message
                statusMessage.className = 'status-message success show';
                statusText.textContent = 'Homework saved successfully!';
            } else {
        ...
        statusText.textContent = 'Error saving homework. Please try again.';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load homework for today's date
    loadHomeworkFromStorage();
    
    // Add event listener to load homework when date changes
